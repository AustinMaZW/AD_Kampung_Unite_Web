...
public class GroupPlanServiceImpl implements GroupPlanService {
	...
	public Boolean quitGroceryPlan(int id) {
		...
		if (groceryList!=null){
			List<HitchRequest> hitchRequests = hrqRepo.findHitchRequestsByHitcherDetailId(groceryList.getHitcherDetail().getId());
			...
			List<CombinedPurchaseList> combinedPurchaseList = cplRepo.findCombinedPurchaseListsByGroupPlanId(groceryList.getGroupPlanGL().getId());
			if (combinedPurchaseList!=null){
				...
				groceryItems.stream().forEach(x->{
					combinedPurchaseList.stream().forEach(y->{
						if(x.getProduct().getId() == y.getProduct().getId()){	//check groceryItems that are in combinedPurchaseList
